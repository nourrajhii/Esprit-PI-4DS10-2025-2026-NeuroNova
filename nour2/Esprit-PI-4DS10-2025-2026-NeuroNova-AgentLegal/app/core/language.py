...
def detect_question_type(question: str) -> str:
    ...
    # ── Rôle du notaire / transaction immobilière → general (no source filter) ─
    if any(k in q for k in [
        'rôle', 'role', 'fonction', 'mission', 'responsabilité',
        'transaction', 'intervention', 'authentique',
        'دور', 'مهمة', 'صلاحية', 'مسؤولية',
    ]) and any(k in q for k in [
        'notaire', 'كاتب عدل', 'عدل',
    ]):
        return "general"

    # ── Documents / acte ────────────────────────────────────
    if any(k in q for k in [
        'document', 'dossier', 'pièce', 'liste des pièces',
        'quels documents', 'quelles pièces', 'fournir',
        'quel papier', 'paperasse',
        'وثيقة', 'ملف', 'وثائق مطلوبة', 'الأوراق',
    ]):
        return "documents"

    # ── Acte notarié / contrat de vente → COC ───────────────
    if any(k in q for k in [
        'acte', 'notaire', 'acheter', 'vendre', 'promesse', 'compromis',
        'contrat de vente', 'acte de vente',
        'بيع', 'شراء', 'كاتب عدل', 'وكالة', 'عقد بيع',
    ]):
        return "coc"
