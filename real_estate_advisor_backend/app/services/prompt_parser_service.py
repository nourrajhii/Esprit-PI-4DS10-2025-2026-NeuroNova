...
KNOWN_CITIES = [
    ...
    # Darija variants
    "tounes", "tunes", "soussa", "sfeks", "naboul", "monastir",
    "beja", "jendouba", "siliana", "zaghouan", "gafsa", "tozeur",
    "kebili", "tataouine", "kasserine", "sidi bouzid",
]

# Darija city normalization map
_DARIJA_CITY_MAP = {
    "tounes": "Tunis", "tunes": "Tunis",
    "soussa": "Sousse",
    "sfeks": "Sfax",
    "naboul": "Nabeul",
    "beja": "Béja",
...
REAL_ESTATE_KEYWORDS = [
    ...
    "rent", "sale", "buy",
    # Darija keywords
    "nheb", "nhb", "nabi", "nabghi", "bghi",  # je veux
    "ekri", "ekra", "nkri", "nkra",            # louer
    "chri", "acheteh",                          # acheter
    "dar", "bit", "falej",                      # maison/appartement
    "ardh", "terrain",
    "f tunis", "f tounes", "fi tunis",
]
...
def extract_city_from_prompt(user_prompt: str) -> str | None:
    ...
    # Check Darija variants first (longer strings first to avoid partial matches)
    for darija, canonical in _DARIJA_CITY_MAP.items():
        if darija in text:
            return canonical
...
def extract_budget_from_prompt(user_prompt: str) -> tuple[float | None, float | None]:
    ...
    # Reassemble numbers with space/dot thousand-separators: "450 000" → 450000, "1 200" → 1200
    def _reassemble(t: str) -> list[float]:
        # Match groups like "450 000" or "1 200 000" (digit blocks separated by spaces/dots)
        compound = re.findall(r"\d{1,3}(?:[\s.]\d{3})+", t)
        results = []
        for c in compound:
            try:
                results.append(float(re.sub(r"[\s.]", "", c)))
            except ValueError:
                pass
        # Also match plain numbers
        for v in re.findall(r"\b\d+(?:[,]\d+)?\b", t):
            try:
                results.append(float(v.replace(",", ".")))
            except ValueError:
                pass
        # Deduplicate and filter plausible real-estate values
        seen = set()
        out = []
        for v in results:
            if v not in seen and v >= 100:
                seen.add(v)
                out.append(v)
        return out

    values = _reassemble(text)
